...
import { fetchPatient, fetchRelationships, fetchObsByPatientAndEncounterType, formatPatientForCard } from "./patient-resources";
...
export function usePatientDashboard(patientUuid?: string) {
    ...
    if (state.isLoadingPatient && state.patientUuid) {
      fetchPatient(state.patientUuid).then(
        async (data) => {
          ...
          active &&
            dispatch({
              patient: await formatPatientForCard(data?.data),
              relationships: relationships,
              obs: obs,
              type: ActionTypes.newPatient,
            })
